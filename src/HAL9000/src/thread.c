...
typedef struct _THREAD_SYSTEM_DATA
{
    ...
    _Guarded_by_(NumberOfThreadsLock)
    ...
    )
{
    ...
    //lab 1
    //InitializeListHead(&m_threadSystemData.NumberOfThreads);
    LockInit(&m_threadSystemData.NumberOfThreadsLock);
    m_threadSystemData.NumberOfThreads = 0;
}

//lab 1
QWORD
ReturnNumberOfThreads(
    void
) {
...
#pragma warning(suppress:4152)
    ...
    pThread->ParentTID = GetCurrentThread()->Id;

    *Thread = pThread;

    //lab 1 ex 1
    LOG("Thread create with id 0x%X and name %s", pThread->Id, pThread->Name);

    INTR_STATE numberState;
    LockAcquire(&m_threadSystemData.NumberOfThreadsLock, &numberState);
    m_threadSystemData.NumberOfThreads += 1;
    LockRelease(&m_threadSystemData.NumberOfThreadsLock, numberState);
